"""TOS-DEV08-I02 ClassroomAssessment security audit constraint extension.

Revision ID: tosd080002
Revises: tosd080001
Create Date: 2026-09-04

Extends security.audit_records CHECK constraints for assessment.classroom.*
actions. Does not alter assessment.classroom_assessments or Content/Teaching
schema.

Executes under AIEOS_SECURITY_SCHEMA_OWNER_ROLE, then restores
AIEOS_SCHEMA_OWNER_ROLE. Downgrade fails closed when Assessment audit evidence
exists.
"""
...
revision: str = "tosd080002"
down_revision: str | None = "tosd080001"
...
_ASSESSMENT_EVIDENCE_BLOCKED = (
    "TOS-DEV08-I02 downgrade refused: ClassroomAssessment security audit "
...
_CONTENT_TEACHING_ASSESSMENT_ACTIONS_SQL = (
    _CONTENT_ACTIONS_SQL.rstrip()
    + ","
    + _TEACHING_ACTIONS_SQL
    + ","
...
def downgrade() -> None:
    ...
    if blocked:
        ...
        op.execute(f"SET LOCAL ROLE {content_owner}")
        raise RuntimeError(_ASSESSMENT_EVIDENCE_BLOCKED)
