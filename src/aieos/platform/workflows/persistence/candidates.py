"""WORKFLOW dispatcher candidate repositories (ADR-AIEOS-045 / PED-I12)."""
...
class WorkflowDispatchCandidate:
    tenant_id: UUID
    eligible_at: datetime


class SqlAlchemyStartIntentCandidateRepository:
    """Calls ``workflow.list_start_intent_candidates`` only.

    Returns ``tenant_id`` + ``eligible_at`` exclusively. Does not set tenant
    context, change role, or read intent payload columns.
    """
...
class SqlAlchemyCommandIntentCandidateRepository:
    """Calls ``workflow.list_command_intent_candidates`` only.
