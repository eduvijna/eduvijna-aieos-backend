...
class DispatcherConfig:
    ...
    start_reconciliation_timeout_seconds: float = 10.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.start_reconciliation_timeout_seconds <= 0:
            raise ValueError("start_reconciliation_timeout_seconds must be positive")
    ...
    async def dispatch_once(self, tenant_id: UUID) -> bool:
        ...
        result = await self._gateway.start_content_review(
            ...
            reconciliation_timeout_seconds=(
                self._config.start_reconciliation_timeout_seconds
            ),
        )
        if result.delivered:
            return self._repository.mark_start_delivered(
                tenant_id=tenant_id,
...
class ContentReviewCommandDispatcher:
    ...
    async def dispatch_once(self, tenant_id: UUID) -> bool:
        ...
        if result.delivered:
            return self._repository.mark_command_delivered(
                tenant_id=tenant_id,
