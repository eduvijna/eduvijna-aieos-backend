...
# Database privilege contract (GCI-I02R2 / GCI-I05R1 / GCI-I06)
...
| Runtime | Ordinary Generic Content DML and synchronous API idempotency DML | LOGIN | Superuser, `BYPASSRLS`, schema ownership, `DELETE` on `content.contents`, `UPDATE`/`DELETE` on `content.content_versions`, `UPDATE`/`DELETE` on `content.review_decisions`, `UPDATE`/`DELETE` on `api.idempotency_records` |
...
- `content.review_decisions` owner == schema-owner role
- `content.reject_review_decision_mutation()` owner == schema-owner role
