# WPI-OCI-I01 / I01R1 — Backend Production OCI Source and Provenance Validation
...
## WPI-OCI-I01R1 hardenings

- CI `backend-production-oci` checks out and builds the **exact PR HEAD**
  (`github.event.pull_request.head.sha`), never GitHub's synthetic
  `refs/pull/<n>/merge` / `github.sha` on `pull_request`.
- Authoritative provenance CLI has **no `--allow-dirty`** bypass.
- Receipt Python/uv/base-image/OS/user/Cmd/labels are **derived from the
  actual image and Dockerfile**, then compared to governed expectations.
- Verifier binds `application_version`, `dockerfile_sha256`, and
  `uv_lock_sha256` to **current repository files**.
- Runtime user must be exactly `10001:10001` (named `aieos` rejected).
- Default command must **exactly** equal `default_command_contract()`.
- Dockerfile build-args are fail-closed (no `=unknown` revision defaults;
  40-char lowercase SHA + VERSION coherence enforced at build).
...
- PR validation builds using the **exact PR HEAD** (`pull_request.head.sha`)
- Post-merge / push-to-main validation uses the **exact push `github.sha`**
